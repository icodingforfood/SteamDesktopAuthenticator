...
namespace Steam_Desktop_Authenticator
{
    public partial class LoginForm : Form
    {
            ...
            while ((response = userLogin.DoLogin()) != LoginResult.LoginOkay)
            {
                switch (response)
                {
                    ...
                    case LoginResult.Need2FA:
                        MessageBox.Show("This account already has a mobile authenticator linked to it.\nRemove the old authenticator from your Steam account before adding a new one.", "µÇÂ¼Ê§°Ü", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        this.Close();
                        return;

                    case LoginResult.BadRSA:
                        MessageBox.Show("µÇÂ¼Ê§°Ü: Steam ·µ»Ø \"BadRSA\".", "µÇÂ¼Ê§°Ü", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        this.Close();
                        return;

                    case LoginResult.BadCredentials:
                        MessageBox.Show("µÇÂ¼Ê§°Ü: ÓÃ»§Ãû»òÃÜÂë²»ÕýÈ·.", "µÇÂ¼Ê§°Ü", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        this.Close();
                        return;

                    case LoginResult.TooManyFailedLogins:
                        MessageBox.Show("µÇÂ¼Ê§°Ü: ¶à´ÎÊ§°ÜµÇÂ¼ ÉÔºóÖØÊÔ.", "µÇÂ¼Ê§°Ü", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        this.Close();
                        return;

                    case LoginResult.GeneralFailure:
                        MessageBox.Show("µÇÂ¼Ê§°Ü: Steam ·µ»Ø \"GeneralFailure\".", "µÇÂ¼Ê§°Ü", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        this.Close();
                        return;
                }
            }
            ...
            while ((linkResponse = linker.AddAuthenticator()) != AuthenticatorLinker.LinkResult.AwaitingFinalization)
            {
                switch (linkResponse)
                {
                    case AuthenticatorLinker.LinkResult.MustProvidePhoneNumber:
                        ...
                        {
                            InputForm phoneNumberForm = new InputForm("ÇëÊäÈëÄãµÄµç»°ºÅÂë£¬¸ñÊ½Èç: +{cC} µç»°. cn, +86 12345678900");
                    ...
                    case AuthenticatorLinker.LinkResult.GeneralFailure:
                        MessageBox.Show("Ìí¼Óµç»°ºÅÂë´íÎó. Steam ·µ»Ø \"GeneralFailure\".");
                        this.Close();
                        return;
                }
            }
        ...
        private void HandleManifest(Manifest man, bool IsRefreshing = false)
        {
            ...
            if (IsRefreshing)
            {
                MessageBox.Show("ÄãµÄµÇÂ¼»á»°ÒÑ¾­Ë¢ÐÂ.");
            }
            else
            {
                MessageBox.Show("ÊÖ»úÈÏÖ¤Æ÷Á´½Ó³É¹¦. ÇëÐ´ÏÂ³·»ØÂë: " + androidAccount.RevocationCode);
            }
            this.Close();
        }
